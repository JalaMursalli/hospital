                ...
                <div class="other-langs">
                    ...
                    @foreach(['az','en','ru'] as $language)
                                        ...
                                        @php
                                            $key = 'slug_'.$language;
                                        @endphp
                                        <a href="/{{$language}}/seminar-details/{{$seminar->$key}}" class="other-lang-item">  {{ strtoupper($language) }}</a>
...
<div class="pageDirection">
    ...
    <a href="{{route('seminar')}}" class="pageDirection-item">{{$settings['seminar']}}</a>
    <span class="active">
        <svg width="15" height="8" viewBox="0 0 15 8" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0.504 0.48H2.344L6.216 4.144L2.344 7.824H0.504L4.376 4.144L0.504 0.48ZM9.129 0.48H10.969L14.841 4.144L10.969 7.824H9.129L13.001 4.144L9.129 0.48Z" fill="#636363"/>
        </svg>
    </span>
    <a href="{{route('showSeminar', ['language'=>app()->getLocale(),'slug'=>$seminar->slug])}}" class="pageDirection-item active">{{$seminar?->title}}</a>
</div>
<div class="blog-detail-container">
        ...
        <div class="blog-detail-img">
            <div class="blog-time">
                ...
                <div class="time-inner">
                    <p>{{$seminar->created_at?->translatedFormat('d F Y')}}</p>
                </div>
            </div>
            <img src="{{$seminar?->image2}}" alt="" class="detailImg">
        </div>
        <h1 class="blogName">{{$seminar?->title}}</h1>
        <div class="blog-text">
    ...
    <div class="blog-detail-right">
        ...
        <div class="otherBlogs">
            <h2>{{$settings['latestSeminars']}}</h2>
            <div class="otherBlogs-items">
                @foreach ( $latestSeminars as $seminar )
                <a href="{{route('showSeminar', ['language'=>app()->getLocale(),'slug'=>$seminar->slug])}}" class="otherBlogItem">
                    <div class="cartImg">
                        <img src="{{$seminar?->image1}}" alt="">
                    </div>
                    <div class="cartBody">
                        <div class="blogDate">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
@push('js')
    <script>
        function search_popular_posts(input){
            ...
            let url = `/${lang}/get-popular-seminars?search=${search_value}`;
