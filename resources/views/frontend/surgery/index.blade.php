...
@section('content')
@include('frontend.surgery.surgery-banner')
<div class="departments-container">
    <div class="departments-head">
        <h2 class="headTitle">{{$settings['surgery']}}</h2>
        <form action="" class="search-department-form">
            <input type="text" placeholder="{{$settings['surgerySearch']}}"  name="search" value="{{request()->search}}">
    ...
    <div class="department-boxes">
        ...
        <a href="{{route('showSurgery', ['language'=>app()->getLocale(),'slug'=>$surgery->slug])}} " class="department-box">
            <div class="icon">
                <img src="{{asset($surgery?->icon)}}" alt="">
            </div>
            <p>{{$surgery?->title}}</p>
        </a>
        @endforeach
