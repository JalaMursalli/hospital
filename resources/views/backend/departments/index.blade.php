                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header align-items-end d-flex justify-content-between">
                           <div>
                            <h4>Şöbələrin siyahısı</h4>
                            <div class="card-header-form">
                                <!-- Search Form -->
                                <form action="{{ route('backend.department.index') }}" method="GET" class="d-flex"
                            ...
                            <a href="{{ route('backend.department.create') }}" class="btn btn-primary">Yeni Şöbə yarat</a>
                        </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    @forelse ($departments as $key => $department)
                                        <tr>
                                            <td>{{ $departments->firstItem() + $key }}</td> <!-- Keeps count on pagination -->
                                            <td>
                                                <img src="{{ asset($department->icon) }}" alt=""
                                                    style="height: 70px; weight: 70px">
                                            </td>
                                            <td>{{ $department->title_az }}</td>
                                            {{-- <td>{{ $department->category?->title_az }}</td> --}}
                                            <td>{{ $department->created_at }}</td>
                                            <td class="text-end">
                                                <div class="d-flex justify-content-end " style="gap: 20px">
                                                    <a href="{{ route('backend.department.edit', $department?->id ?? '') }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="text-center">Şöbə tapılmadı</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
